...
local function doFixLag()
    ...
    -- ════════════════════════════════
    -- Sử dụng offset-only position để drag chính xác
    local toggle = Instance.new("TextButton", gui)
    toggle.Size = UDim2.new(0, 62, 0, 62)
    toggle.Position = UDim2.new(0, 20, 1, -82)  -- offset-only, tránh lỗi scale+offset
    ...
    Instance.new("UICorner", toggle).CornerRadius = UDim.new(1, 0) -- full circle
    ...
    -- ── Drag logic (offset-only, không mix scale) ──
    local dragging   = false
    local dragStart  = nil  -- mouse position khi bắt đầu
    local dragOrigin = nil  -- toggle.Position offset khi bắt đầu
    ...
    if BF_IDs[game.PlaceId] then
        task.spawn(function()
            local ok, err = pcall(function()
                loadstring(game:HttpGet(
                    "https://raw.githubusercontent.com/leduccuong01091987-collab/Chanbomayde/refs/heads/main/notify.lua"
                ))()
            end)
            if not ok then warn("BeeZ: Lỗi load script: "..tostring(err)) end
        end)
    end
end
